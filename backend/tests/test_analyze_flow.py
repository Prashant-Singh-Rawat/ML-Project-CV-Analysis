...
def test_analyze_flow_success():
    ...
    assert "placement_probability" in json_data["hiring_analysis"]
    assert "placement_status" in json_data["hiring_analysis"]
    
def test_analyze_flow_invalid_file():
    """Test that submitting a non-PDF file returns a 400."""
    files = {'file': ('resume.txt', b'This is just a text file.', 'text/plain')}
    data = {'target_company': 'Google', 'cgpa': '8.5'}
