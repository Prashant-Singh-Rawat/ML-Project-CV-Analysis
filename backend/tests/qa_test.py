...
for module_name in [
    "spacy", "pdfplumber", "fpdf", "passlib", "passlib.context",
    "jose", "jose.jwt", "pythonjsonlogger", "sentence_transformers"
]:
    sys.modules[module_name] = MagicMock() if module_name not in sys.modules else sys.modules[module_name]


# Setup sentence_transformers mock utilities
...
for idx, case in enumerate(test_cases, 1):
    ...
    confidence = sem_res["match_details"][0]["confidence"] if sem_res["match_details"] else 0.0
    
    status = "SUCCESS" if (sem_pct > 0 and idx < 5) or (sem_pct == 0 and idx == 5) else "FAILED"
    ...
    print(f"  New Semantic Match: Matched: {sem_matched} | Score: {sem_pct:.1f}% | Similarity: {confidence}% (Engine: {engine})")
